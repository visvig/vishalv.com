...
tags:
  ...
  - "adjacency-matrix"
draft: false
summary: "Research notes on CS168: The Modern Algorithmic Toolbox Lectures #11: Spectral Graph Theory, I."
---
...
> D(i, i) is the degree of the ith node
...
> consider what happens to a vector when we multiply it by L: if Lv = w
...
> ith element of the product Lv is the sum of the differences between v(i) and the indices of v corresponding to the neighbors of i in the graph G
...
![research-notes/images/roughgardenCS168ModernAlgorithmic/image-2-x149-y281.png](/static/research-notes/images/roughgardenCS168ModernAlgorithmic/image-2-x149-y281.png)
...
> if one interprets the vector v as assigning a number to each vertex in the graph G, the quantity vtLv is exactly the sum of the squares of the differences between the values of neighboring nodes
...
> if one were to place the vertices of the graph on the real numberline, with the ith node placed at location v(i), then vtLv is precisely the sum of the squares of the lengths of all the edges of the graph
...
> L is a real-valued symmetric matrix, all of its eigenvalues are real numbers, and its eigenvectors are orthogonal to eachother
...
> The eigenvalues of L contain information about the structure (or, more precisely, information about the extent to which the graph has structure)
...
> The laplacian always has at least one eigenvalue that is 0
...
> multiplicity of the zeroth eigenvalue reveals the number of connected components of the graph
...
> The number of zero eigenvalues of the Laplacian LG (i.e. the multiplicity of the 0 eigenvalue) equals the number of connected components of the graph G
...
> vtLv = P  (i,j)∈E,i&lt;j(v(i) − v(j))2 is the sum of the squares of the distances between neighbors
...
> eigenvectors corresponding to the lowest eigenvalues correspond to vectors for which neighbors have similar values
...
> smallest nonzero eigenvector will be the vector that minimizes this squared distance between neighbors, subject to being a unit vector that is orthogonal to all zero eigenvectors
...
> The maximum eigenvalue max||v||=1 vtLv will try to maximize the discrepancy between neighbors’ values of v
...
![research-notes/images/roughgardenCS168ModernAlgorithmic/image-4-x99-y245.png](/static/research-notes/images/roughgardenCS168ModernAlgorithmic/image-4-x99-y245.png)
...
> Spectral embeddings of the cycle on 20 nodes (top two figures), and the 20 × 20 grid (bottom two figures)
...
> The left two plots show the embeddings onto the eigenvectors corresponding to the second and third smallest eigenvalues; namely, the ith node is plotted at the point (v2(i), v3(i)) where vk is the eigenvector corresponding to the kth largest eigenvalue
...
> The right two plots show the embeddings onto the eigenvectors corresponding to the largest and second–largest eigenvalues
...
> the embedding onto the smallest two eigenvectors really does correspond to the “natural”/“intuitive” way to represent the graph in 2-dimensions
...
> partition the graph into several large components so as to minimize the number of edges that cross between the components
...
> metrics for the quality of a given cluster or partition of a graph, and give some quantitative bounds on these metrics in terms of the second eigenvalue of the graph Laplacian
...
> eigenvectors corresponding to small eigenvalues are, in some sense, trying to find good partitions of the graph
...
> If two radio stations have overlapping regions of broadcast, then they cannot be assigned the same bandwidth (otherwise there will be interference for some listeners). On the other hand, if two stations are very far apart, they can broadcast on the same bandwidth without worrying about interference from each other
...
> nodes of the graph will represent radio stations, and there will be an edge between two stations if their broadcast ranges overlap
...
> problem of finding a k-color, or even deciding whether a k-coloring of a graph exists, is NP-hard
...
> k-coloring heuristic as follows:
...
> 1) plot the embedding of the graph onto the top 2 or 3 eigenvectors of the Laplacian
...
> 2) locally partition the points in this space into k regions, e.g. using k-means, or a kd-tree
...
> 3) assign all the points in each region the same color
