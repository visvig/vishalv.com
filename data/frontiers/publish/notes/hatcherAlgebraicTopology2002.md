...
tags:
  ...
  - "mapping-cone"
draft: false
summary: "Research notes on Algebraic topology."
---
...
![research-notes/images/hatcherAlgebraicTopology2002/image-6-x61-y681.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-6-x61-y681.png)
...
> CW complexes have proved over time to be the most natural class of spaces for algebraic topology
...
> algebraic topology separates naturally into the two broad channels of homology and homotopy
...
> tentatively titled ‘Spectral Sequences in Algebraic Topology’ and is referred to herein as [SSAT]
...
> vector bundles, characteristic classes, and K–theory, which will be largely independent of [SSAT] and also of much of the present book. This is referred to as [VBKT]
...
> reader should know about quotient spaces, or identification spaces
...
> ∆ complexes, which are a mild generalization of the classical notion of a simplicial  complex
...
> two spaces to be equivalent if they have ‘the same shape’ in a sense that is much broader than homeomorphism
...
> thin letter is a subspace of the thick letter, and we can continuously shrink the thick letter to the thin one
...
![research-notes/images/hatcherAlgebraicTopology2002/image-10-x288-y180.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-10-x288-y180.png)
...
> shrink X to X by sliding each point of X − X into  X along the line segment
...
> think of this shrinking process as taking place during a time interval 0 ≤ t ≤ 1
...
> ft : X→X parametrized by t ∈ I =  [0, 1] , where ft(x) is the point to which a given point x ∈ X has moved at time t
...
> deformation  retraction of a space X onto a subspace A is a family of maps ft : X→X , t ∈ I , such  that f0 = 11 (the identity map), f1(X) = A , and ft || A = 11 for all t
...
> deformation retraction of a M ̈obius band onto its core circle
...
![research-notes/images/hatcherAlgebraicTopology2002/image-11-x221-y458.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-11-x221-y458.png)
...
![research-notes/images/hatcherAlgebraicTopology2002/image-11-x75-y458.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-11-x75-y458.png)
...
> disk with two smaller open subdisks removed shrinks to three different subspaces
...
> map f : X→Y , the mapping  cylinder Mf is the quotient space of the disjoint union (X × I) ∐ Y obtained by identifying each (x, 1) ∈ X × I with f (x) ∈ Y
...
![research-notes/images/hatcherAlgebraicTopology2002/image-11-x238-y258.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-11-x238-y258.png)
...
> mapping cylinder Mf deformation retracts to the subspace Y by sliding each point (x, t) along the segment  \{x\}× I ⊂ Mf to the endpoint f (x) ∈ Y
...
> thick X deformation retracts to the thin X , which in turn deformation retracts to the point of intersection of its two crossbars. The net result is a deformation retraction of X onto a point
...
> deformation retraction ft : X→X is a special case of the general notion of a  homotopy, which is simply any family of maps ft : X→Y , t ∈ I , such that the associated map F : X × I→Y given by F (x, t) = ft(x) is continuous
...
> two maps f0, f1 : X→Y are homotopic if there exists a homotopy ft connecting them, and one writes f0 ≃ f1
...
> deformation retraction of X onto a subspace A is a homotopy from the identity map of X to a retraction of X onto A , a map r : X→X such that  r (X) = A and r || A = 11
...
> retraction as a map X→A  restricting to the identity on the subspace A ⊂ X
...
> retraction is a map r : X→X with r 2 = r
...
> Retractions are the topological analogs of projection operators
...
> space X always retracts onto any point x0 ∈ X via the constant map sending all of X to x0 , but a space that deformation retracts onto a point must be path-connected since a deformation retraction of X to x0 gives a path joining each x ∈ X to x0
...
> there are path-connected spaces that do not deformation retract onto a point
...
> homotopy ft : X→Y whose  restriction to a subspace A ⊂ X is independent of t is called a homotopy relative  to A , or more concisely, a homotopy rel A
...
> deformation retraction of X onto  A is a homotopy rel A from the identity map of X to a retraction of X onto A
...
> ft : X→X , then if  r : X→A denotes the resulting retraction and i : A→X the inclusion, we have r i = 11  and ir ≃ 11 , the latter homotopy being given by ft
...
> map f : X→Y is called a homotopy equivalence if there is a map g : Y →X such that  f g ≃ 11 and gf ≃ 11
...
> spaces X and Y are said to be homotopy equivalent or to  have the same homotopy type
...
> this is an equivalence relation, in contrast with the nonsymmetric notion of deformation retraction
...
> the three graphs are all homotopy equivalent
...
> take Z to be the mapping cylinder Mf of any homotopy equivalence f : X→Y
...
> identity map of the space be nullhomotopic, that is, homotopic to a constant map
...
> slightly weaker than saying the space deformation retracts to a point
...
> example of a 2 dimensional subspace of R  3 , known as the  house with two rooms
...
![research-notes/images/hatcherAlgebraicTopology2002/image-13-x75-y510.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-13-x75-y510.png)
...
> The resulting space X thus consists of three horizontal pieces homeomorphic to annuli plus all the vertical rectangles that form the walls of the two chambers
...
> consider a closed ε neighborhood N(X) of X . This clearly deformation retracts onto X if ε is sufficiently small
...
> N(X) is homeomorphic to D  3 , the unit ball in R  3
...
> X deformation retracts to a point
...
> an orientable surface Mg of genus g can be constructed from a polygon with 4g sides
...
> 4g edges of the polygon become a union of 2g circles in the surface, all intersecting in a single point
...
> interior of the polygon can be thought of as an open disk, or a 2 cell, attached to the union of the 2g circles
...
> union of the circles as being obtained from their common point of intersection, by attaching 2g open arcs, or 1 cells
...
![research-notes/images/hatcherAlgebraicTopology2002/image-14-x240-y372.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-14-x240-y372.png)
...
> quotient space of the disjoint union X n−1 ∐  α Dαn
...
> one can continue indefinitely, setting X = ⋃  n Xn
...
> space X constructed in this way is called a cell complex or CW complex
...
> X = Xn for some n , then X is said to be finite-dimensional, and the smallest such n is the dimension of X , the maximum dimension of cells of X
...
> 1 dimensional cell complex X = X1 is what is called a graph
...
> Euler characteristic, which for a cell complex with finitely many cells is defined to be the number of even-dimensional cells minus the number of odd-dimensional cells
...
> Euler characteristic of a cell complex depends only on its homotopy type, so the fact that the house with two rooms has the homotopy type of a point implies that its Euler characteristic must be 1
...
> sphere Sn has the structure of a cell complex with just two cells, e0  and en , the n cell being attached by the constant map Sn−1→e0
...
> Sn as the quotient space Dn/∂Dn
...
> Real projective n space RPn is defined to be the space of all lines  through the origin in Rn+1 . Each such line is determined by a nonzero vector in Rn+1 , unique up to scalar multiplication
...
> RPn is topologized as the quotient space of  Rn+1 − \{0\} under the equivalence relation v ∼ λv for scalars λ ≠ 0
...
> RPn is also the quotient space Sn/(v ∼ −v)
...
> RPn is the quotient space of a hemisphere Dn with antipodal points of ∂Dn identified
...
> ∂Dn with antipodal points identified is just RPn−1
...
> RPn is obtained from RPn−1 by attaching an n cell, with the quotient projection Sn−1→RPn−1 as the attaching map
...
> RP∞ as the space of lines through the origin in R∞ = ⋃  n Rn
...
> Complex projective n space CPn is the space of complex lines through  the origin in Cn+1 , that is, 1 dimensional vector subspaces of Cn+1
...
> quotient of the unit sphere S  2n+1  ⊂C  n+1  with v ∼ λv for |λ| = 1
...
> CP  n  as a quotient space of the disk D  2n  under the identifications v ∼ λv for v ∈ ∂D  2n
...
> CP  n  is obtained from CP  n−1  by attaching a cell e  2n  via the quotient map S  2n−1  →  CP  n−1  . So by induction on n we obtain a cell structure CP  n  =e  0  ∪e  2  ∪ ··· ∪ e  2n  with cells only in even dimensions
...
> Each cell e  n α  in a cell complex X has a characteristic map Φ  α  :D  n α  →  X which extends the attaching map φ  α  and is a homeomorphism from the interior of D  n α  onto e  n α
...
> canonical cell structure on S  n  described in Example 0.3, a characteristic map for the n cell is the quotient map  D n  →  S n  collapsing ∂D  n  to a point
...
> subcomplex of a cell complex X is a closed subspace A ⊂ X that is a union of cells of X
...
> A is a cell complex in its own right
...
> pair (X, A) consisting of a cell complex X and a subcomplex A will be called a CW pair
...
> natural inclusions S  0  ⊂S  1  ⊂ ··· ⊂ S  n  , but these subspheres are not subcomplexes of S  n  in its usual cell structure with just two cells
...
> equatorial S  k−1  by attaching two k cells, the components of S  k  −S  k−1
...
> two-to-one quotient map S  ∞ →  RP  ∞  that identifies antipodal points of S  ∞  identifies the two n cells of S  ∞  to the single  n cell of RP  ∞
...
> if we start with S1 with its minimal cell structure and attach to this a 2 cell by a map S1→S1 whose image is a nontrivial subarc of S1 , then the closure of the 2 cell is not a subcomplex since it contains only a part of the 1 cell.
...
> flexibility to allow many natural constructions
...
> X × Y has the structure of a cell  complex with cells the products eαm × eβn
...
> general CW complexes X and Y there is one small complication: The topology on X × Y as a cell complex is sometimes finer than the product topology, with more open sets than the product topology has, though the two topologies coincide if either X or Y has only finitely many cells, or if both X and Y have countably many cells
...
> cells of X/A are the cells of X − A plus one new 0 cell, the image of A in X/A
...
> suspension SX is the quotient of  X × I obtained by collapsing X × \{0\} to one point and X × \{1\} to another point
...
![research-notes/images/hatcherAlgebraicTopology2002/image-17-x460-y18.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-17-x460-y18.png)
...
> X = Sn , when SX = Sn+1 with the two ‘suspension points’ at the north and south poles of  Sn+1 , the points (0, ··· , 0, ±1)
...
> not only spaces but also maps can be suspended
...
> cone CX is the union of all line segments joining points of X to an external  vertex
...
> suspension SX is the union of all line segments joining points of X to two external vertices
...
> given X and a second space Y , one can define the space of all line segments joining points in X to points in Y . This is the join X ∗ Y
...
> an iterated join X1 ∗ ··· ∗ Xn can be constructed as the space of formal linear combinations t1x1 + ··· + tnxn with 0 ≤ ti ≤ 1 and t1 + ··· + tn = 1
...
> join of two points is a line segment
...
> join of three points is a triangle,
...
> join of four points is a tetrahedron
...
> join of n points is a convex polyhedron of dimension n − 1 called a simplex
...
> n points are the n standard basis vectors for Rn , then their join is the (n − 1) dimensional simplex
...
> join X1 ∗ ··· ∗ Xn is the union of 2n copies of the simplex ∆n−1
...
> radial projection from the origin gives a homeomorphism between X1 ∗ ··· ∗ Xn and Sn−1
...
> X and Y are CW complexes, then there is a natural CW structure on X ∗ Y having the subspaces X and Y as subcomplexes, with the remaining cells being the product cells of X × Y × (0, 1)
...
> spaces X and  Y with chosen points x0 ∈ X and y0 ∈ Y , then the wedge sum X ∨ Y is the quotient  of the disjoint union X ∐ Y obtained by identifying x0 and y0 to a single point
...
> S1 ∨ S1 is homeomorphic to the figure ‘8’
...
> smash product X ∧ Y is then defined to be the quotient X × Y /X ∨ Y
...
> Sm ∧Sn has a cell structure with just two cells, of dimensions 0 and m+n , hence Sm ∧Sn = Sm+n
...
> when m = n = 1 we see that collapsing longitude and meridian circles of a torus to a point produces a 2 sphere
...
> main tool we used for constructing homotopy equivalences was the fact that a mapping cylinder deformation retracts onto its ‘target’ end
...
> one can often produce homotopy equivalences between rather different-looking spaces
...
> first involves collapsing certain subspaces to points
...
> second involves varying the way in which the parts of a space are put together
...
> collapsing the middle edge of the first and third graphs produces the second graph
...
> until all edges of X are loops, and then each component of X is either an isolated vertex or a wedge sum of circles
...
> two such graphs, having only one vertex in each component, can be homotopy equivalent if they are not in fact just isomorphic graphs
...
> wedge sum ∨  m S1 of m circles is not  homotopy equivalent to ∨  n S1 if m ≠ n
...
> ∨  m S1 has Euler characteristic 1−m
...
> it  is a rather nontrivial theorem that the Euler characteristic of a space depends only on its homotopy type
...
> algebraic invariant that works equally well for graphs, and whose rigorous development requires less effort than the Euler characteristic, is the fundamental group of a space
...
![research-notes/images/hatcherAlgebraicTopology2002/image-20-x344-y21.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-20-x344-y21.png)
...
> X/A and X/B are homotopy equivalent to X
...
> S2/S0 and S1 ∨ S2 are homotopy equivalent
...
> intersection points are then the 0 cells,
...
> 1 cells are the rest of the longitudinal circle and the boundary circles of the meridional disks
...
> 2 cells are the remaining regions of the torus and the interiors of the meridional disks
...
![research-notes/images/hatcherAlgebraicTopology2002/image-21-x75-y468.png](/static/research-notes/images/hatcherAlgebraicTopology2002/image-21-x75-y468.png)
...
> X be a CW complex and x0 ∈ X a 0 cell.  Inside the suspension SX we have the line segment \{x0\}× I , and collapsing this to a point yields a space ΣX homotopy equivalent to SX , called the reduced suspension of X
...
> ordinary suspension SX is the union of two spheres intersecting along the arc \{x0\}× I
...
> reduced suspension ΣX is S2 ∨ S2
...
> Σ(X ∨ Y ) = ΣX ∨ ΣY for arbitrary CW complexes X and Y
...
> reduced suspension ΣX is actually the same as the smash product X ∧ S1
...
> start with a space X0 and another space X1 that we wish to attach to X0 by identifying the points in a subspace A ⊂ X1 with points of X0
...
> map f : A→X0 , for then we can form a quotient space of X0 ∐ X1 by identifying each point a ∈ A with its image f (a) ∈ X0
...
> X0 ⊔f X1 , the space X0 with X1 attached along A via f
...
> mapping cone Cf = Y ⊔f CX where CX is the cone (X × I)/(X × \{0\}) and we attach this to Y along X × \{1\} via the identifications (x, 1) ∼ f (x)
...
> If (X1, A) is a CW pair and the two attaching maps f , g : A→X0 are homotopic, then  X0 ⊔f X1 ≃ X0 ⊔g X1
...
> attaching map is homotopic to a constant map, and attaching S2 to S1 via a constant map of A yields S1 ∨ S2
...
> obtained from a circle by attaching n 2 spheres along arcs, so the necklace is homotopy equivalent
...
> slide these attaching points around the circle until they all coincide, producing the wedge sum
...
> X/A ≃ X ∪ CA , the mapping cone of the inclusion A  ֓  X
